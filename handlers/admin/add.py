...
async def process_body_back(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        await message.answer(
            f"Change title from <b>{data['title']}</b>?", reply_markup=back_markup()
        )
...
async def process_image_url(message: Message, state: FSMContext):
    if message.text == back_message:
        ...
        async with state.proxy() as data:
            await message.answer(
                f"Change description from <b>{data['body']}</b>?",
...
async def process_confirm_back(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        await message.answer(
            f"Change price from <b>{data['price']}</b>?", reply_markup=back_markup()
        )
...
async def show_products(m, products, category_idx):
    ...
    await m.answer("Want to add or delete something?", reply_markup=markup)
