...
async def process_catalog(message: Message):
    await message.answer(
        "Choose a collection to browse our fine art:",
...
async def category_callback_handler(query: CallbackQuery, callback_data: dict):
    ...
    await query.answer("Available artworks in this collection.")
...
async def add_product_callback_handler(query: CallbackQuery, callback_data: dict):
    ...
    await query.answer("Artwork added to your cart!")
    await query.message.delete()


async def show_products(m, products):
    if len(products) == 0:
        await m.answer("No artworks available in this collection.")
